...
//**********************************FOR PROCESSES ONLY************************************
//here we are also going to initialize the full_process_table, if it has not been done yet
//ie, if this is the very first task created (probably called from inside execv or thread_create)
void fptInitialize(){

        int i;
        //get space for memory
        full_process_table = kmalloc(128 * sizeof(struct thread*));

        //initialize all valuse to null first, for easy comparisons later
        for(i = 0; i < 128; i++){
                full_process_table[i] = NULL;
...
thread_create(const char *name)
{
	...
	//initialize pid, exit status, and parent pointer: start set to 0, -2 and NULL respectively
	//so that this function can still be used for threads
	//ie, threads should not be in full process table or have a valid parent or exit status
	thread->pid = 0;

	//all exit status codes are 0 or above, therefore if this number is < 0, the process 
	//has not yet exited
	thread->exit_status = -1;

	//the thread parent can be NULL unless it has been created from a fork, but we will adjust
	//the parent from there if necessary
	thread->parent = NULL;

	//0 for running, etc process. 1 for exited process
	thread->pros_exit = 0;
	thread->numchilds = 0;
...
thread_destroy(struct thread *thread)
{
	...
	//free extra stuff added to make a thread a process
	thread->parent = NULL;
	thread->pros_exit = 1;
