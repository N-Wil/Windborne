...
/*Global array for process ids, size 128.
  * the entries are the names of active processes, and their indexes will be used as the id.
  * We dont want a process to have pid 0, so when a pid is asked for it will actually be 
  * index + 1
  */
struct thread** full_process_table;
 //global for total number of processes, as this will not be the same as array length
int numprocesses;

void fptInitialize(void);
...
struct thread {
	...
	//extra things added to make a thread a process; pid, exit status, parent process,
	//number of children (to help with waitpid and exit)
	pid_t pid;
	int exit_status;
